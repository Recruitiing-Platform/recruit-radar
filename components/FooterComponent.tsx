...
import { FooterDataInterface } from '@/interfaces/HomepageInterface';
...
const FooterComponent = (props: Props) => {
  return (
    <div className="bg-[#001233]">
      ...
      <div className="px-4 md:px-16 3xl:px-56 flex sm:flex-col lg:flex-row gap-14 md:gap-0 items-start justify-between pt-16 text-recLight">
        {/* Get in touch */}
        ...
        <div className="flex flex-col md:hidden">
          <h3 className="font-recBold 3xl:text-headerSix">
        ...
        {/* Services */}
        <div className="flex flex-wrap gap-12 lg:pl-32 md:pt-10 lg:pt-0">
          {footerData.map((footer: FooterDataInterface, index: number) => {
            return (
              <div className="space-y-5" key={index}>
                <h4 className="uppercase text-recLight font-recBold text-headerSix 3xl:text-[26px] tracking-[6.16px]">
                  {footer.header}
                </h4>
                {footer.subheaders.map((sub: string, index: number) => {
                  return (
                    <Link
                      key={index}
                      href="/recruiter"
                      className="block text-[20px] 3xl:text-headerFive font-recRegular hover:underline hover:font-recMedium transition-all transform duration-300"
                    >
                      {sub}
                    </Link>
                  );
          ...
          {/* socials */}
          <div>
            <h4 className="uppercase text-recLight font-recBold text-headerSix 3xl:text-[26px] tracking-[6.16px] pb-5">
              follow us
            </h4>
      ...
      <div className="text-[10px] md:text-textSmall 3xl:text-textBody text-center text-recLight flex flex-wrap items-center px-5 gap-2.5 justify-start md:justify-center py-14 md:pb-10">
        ...
        <Link
          href=""
          className="hover:underline hover:font-recSemiBold text-[10px] md:text-textSmall"
        >
          Privacy Policy
        </Link>
        <span className="w-2 h-2 bg-[#FA949D] rounded-full"></span>
