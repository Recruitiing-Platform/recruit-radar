...
import { navData, navEmployerData } from '@/data';
...
const EmployerMobileNavComponent = (props: Props) => {
  ...
  return (
    <div
      className={`sm:block lg:hidden ${
        scrolled
          ? 'shadow-[2px_2px_2px_2px_rgba(198,198,198,0.07)] sticky top-0 z-[200]'
          : ''
      }`}
    >
      <motion.nav
        className={`dark:bg-[#000611] bg-recLight flex items-center justify-between px-4 md:px-10 pt-6 pb-3 ${
          isOpen ? 'w-screen top-0 z-10 fixed h-16' : 'sticky top-0'
        } ${
          scrolled
            ? 'shadow-[2px_2px_2px_2px_rgba(198,198,198,0.07)] sticky top-0 z-[100]'
            : ''
        }`}
      >
        {/* Logo */}
      ...
      {isOpen && (
        <div className="bg-recLight dark:bg-[#000611] absolute top-0 right-0 w-[70%] h-screen z-20 pt-10">
          <div className="absolute right-4" onClick={() => setIsOpen(false)}>
            <X />
          </div>
          {/* Nav Items */}
          <ul className="flex flex-col items-center justify-center gap-8 py-28">
            {navEmployerData.map((data: NavDataInterface) => (
              <>
                <li
                  className={
                    pathname === data.link
          ...
          <Button
            variant={'outline'}
            className="font-recSemiBold block mx-auto"
...
export default EmployerMobileNavComponent;
